...
transform_touchstone_country <- function(e) {
  ...
  touchstone_country_db <- DBI::dbGetQuery(con, sprintf("
    SELECT * FROM touchstone_country
     WHERE touchstone IN %s", sql_in(unique(touchstone_country$touchstone))))

  touchstone_country_db$mash <- paste(touchstone_country_db$touchstone,
                                      touchstone_country_db$disease,
                                      touchstone_country_db$country, sep = '#')
  ...
  touchstone_country$id <- touchstone_country_db$id[match(
    touchstone_country$mash, touchstone_country_db$mash)]

  touchstone_country$mash <- NULL
  touchstone_country$already_exists_db <- !is.na(touchstone_country$id)

  if (any(is.na(touchstone_country$id))) {
    which_nas <- which(is.na(touchstone_country$id))
    nid <- next_id(con, "touchstone_country", "id")
    touchstone_country$id[which_nas] <- seq(from = nid, by = 1,
