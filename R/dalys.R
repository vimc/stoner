...
##' stochastic. The columns for both kinds will include `country` (which may be a Montagu
##' numerical id, or the 3-letter code), `year`, `age`, and a number of burden outcomes.
##' Stochastic data will also have a `run_id`.
...
##' interpolated to yearly for age and time. This takes a bit of time, so the resulting
##' data is part of what this function returns, and the life table part can be
##' provided here to speed things up. If provided, it should be a data.frame with
##' the columns `value` and `.code`; the latter is in the format country-year-age. The
##' type of country in `.code` should match with the type of the country in `data`. The years
##' and ages will be interpolated down to single-years.
##' @param year_min The first year of the range in which to calculate DALYs. (Default 2000)
##' @param year_max The final year of the range in which to calculate DALYs. (Default 2100)
##' @return A list with two named components; `life_table` is the result of calling the
##' life-table function, which can then be specified in subsequent calls to `stoner_calculate_dalys`,
...
##' @description Normally this will be called internally from `stoner_calculate_dalys`, but
...
##' @param year_min The first year of the range in which to calculate DALYs. (Default 2000)
##' @param year_max The final year of the range in which to calculate DALYs. (Default 2100)
##' @param char_countries A logical that if TRUE specifies that the countries in the resulting life table
##' should be expressed in 3-character form, or otherwise numerical.
##' @return A data.frame with columns `value` and `.code`. The value is the expected years
...
##' @description When a group has uploaded a burden estimate set without DALYs, this
##' function can retrieve that burden estimate set, extend it with DALYs, and write a
...
##' @param touchstone If specified, the touchstone (including version) for the set.
##' @param scenario If specified, the name of the scenario for the set.
##' @param burden_estimate_set_id Alternatively, if modelling_group, disease, touchstone and
##' scenario are NULL, the numerical id of a particular burden estimate set can be specified here,
##' to extend that estimate set with DALYs. Otherwise, leave as NULL to look up by the other four
##' fields.
##' @param output_file If provided, then write the output CSV with the additional DALYs field
##' to this filename. The file will be ready to be uploaded to Montagu.
##' @param life_table If provided, then re-use the life table provided by a previous call
##' to this function. Otherwise, it can be left at NULL to generate the life-table.
##' @param year_min The first year of the range in which to calculate DALYs. (Default 2000)
##' @param year_max The final year of the range in which to calculate DALYs. (Default 2100)
##' @return A list with two components, `data` is a data.frame of the complete DALYs extended
##' data, and `life_table` is the life table used for calculating the DALYs, should that be
##' required again.
stoner_dalys_for_db <- function(con, dalys_params, modelling_group = NULL, disease = NULL, touchstone = NULL,
  ...
  if (is.null(burden_estimate_set_id)) {
    ...
    list(touchstone, disease, modelling_group, scenario))$current_burden_estimate_set
  }
  ...
  # And add columns (disease, country_name) necessary for uploading back into Montagu.

  data_dalys$data$disease <- disease

  countries <- DBI::dbGetQuery(con, "SELECT * FROM country")
  data_dalys$data$country_name <- countries$name[match(data_dalys$data$country, countries$id)]

  # Re-order columns for Montagu.

  cols <- names(data_dalys$data)
  cols <- cols[!cols %in% c("disease", "year", "age", "country", "country_name", "cohort_size")]
  data_dalys$data <- data_dalys$data[, c("disease", "year", "age", "country", "country_name", "cohort_size", cols)]
