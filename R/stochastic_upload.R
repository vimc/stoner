...
##' @param file A pq or csv file generated by stone_stochastic_process
...
read_processed_stochastic_data <- function(file, is_cohort) {
  ...
  if (type == "csv") {
    ...
    read_stochastic_pq(file, is_cohort)
  } else {
    stop(paste0("Can only read csv or pq format stochastic data, got ", type))
  }
}
...
read_stochastic_pq <- function(file, is_cohort) {
  if (is_cohort) {
    expected_cols <- c("cohort", "country", "run_id")
  } else {
    expected_cols <- c("year", "country", "run_id")
  ...
  if (anyNA(match(expected_cols, colnames(data)))) {
    stop("Columns in pq file not as expected")
  }
  data
}
