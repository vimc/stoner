...
#### touchstone_country
  ...
  * In the `extract-csv` phase, read `meta/touchstone_country.csv` if it exists. This csv has columns
    `(touchstone,diseases,countries)`, where `diseases` and `countries` can be semi-colon separated.
  * The extract phase also queries the database for diseases, countries and touchstones referred to 
    in the csv.
...
#### touchstone_country
  * Check all diseases and countries in the CSV are known to the database. `stoner` does 
    not currently support adding of new diseases or countries.
  * Check that all touchstones referred to in the CSV exist either in the database, or that they will also
    be added during the current update.
  * Also check basic format of the CSV.
...
#### touchstone_country
  * For each row, all combinations of the (semi-colon-separated) countries and diseases are expanded into 
    a proposed append to the `touchstone_country` database table.
  * `id` is assigned - any combinations that already exist in the database table will inherit the `id` from
    the database; new rows will be assigned a new negative id, and flagged as rows to be added.
  * Any duplicates that are found within the incoming CSV will cause an abort.
...
#### touchstone_country
  ...
  * Nothing further to test here; all useful tests are in earlier code.
...
#### touchstone_country
  ...
  * Add all new rows to the database. This is assumed to be an incremental update; it is not possible
    to remove countries from the touchstone_country database with stoner at present.
