...
`stoner` first tests that all the items in the `extracted_data` are recognised by
`stoner`. Not all have to be present, but every present item `stoner` must recpognise
in order to process it. Some of the items will be provided by the user, others will
have been added by `stoner` with database queries. The supported items are:

  * touchstone (stoner)
  * touchstone_csv (user)
  * touchstone_name (stoner)
  * touchstone_name_csv (user)

After this, tests are carried out for each part of the touchstone, as follows. All items
